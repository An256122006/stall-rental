...
import { messageApi, bookingApi, callApi } from '../api/rentalApi';
import { useAppSelector } from '../store/store';
import type { User, ChatMessage, Booking, CallSession } from '../types';
...
export default function ChatCustomersPage() {
  ...
  // Fetch contacts and bookings on mount
  useEffect(() => {
    const initPage = async () => {
      try {
        ...
        if (contactsRes.data.length > 0) {
          ...
          setActiveContactId(contactsRes.data[0].id || null);
        }
      } catch (err) {
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Optimistically append message to screen
  ...
  const getCustomerBooth = (customerId?: number) => {
    if (!customerId) return 'Chưa gán';
    const b = bookings.find(x => x.customer.id === customerId && x.status !== 'CANCELLED');
    return b ? b.booth.code : '—';
  };
  ...
  if (loading) {
    return (
      <div className="chat-customers-page">
        <h2 style={{ fontSize: '26px', fontWeight: '800', color: 'var(--color-slate-900)' }}>Hỗ trợ & Chat với Khách thuê</h2>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '32px' }}>
        <div>
          <h2 style={{ fontSize: '26px', fontWeight: '800', color: 'var(--color-slate-900)', letterSpacing: '-0.5px' }}>
            Hỗ trợ & Chat với Khách thuê
          </h2>
          <p style={{ color: 'var(--color-slate-600)', fontSize: '14.5px', marginTop: '4px', fontWeight: 500 }}>
            Hộp thư trao đổi trực tiếp với khách thuê thuộc khu vực phụ trách.
          </p>
        </div>
      </div>
      ...
      <div className="chat-page-container">
        ...
        <div className="chat-sidebar">
          <div className="chat-sidebar-header">
            <h3 style={{ fontSize: '15px', fontWeight: '800', color: 'var(--color-slate-800)' }}>Danh sách liên hệ</h3>
          </div>
          <div className="chat-customer-list">
            {contacts.map(c => {
              const isActive = c.id === activeContactId;
              const initials = c.fullName ? c.fullName.split(' ').map(n => n[0]).join('').slice(0, 2).toUpperCase() : 'KH';
              const isContactAdmin = c.role === 'ROLE_ADMIN';
              return (
                <div 
                  key={c.id} 
                ...
                >
                  <div className="chat-avatar-wrapper">
                    <div className="chat-avatar" style={{ background: isContactAdmin ? 'var(--color-danger)' : 'var(--color-slate-200)' }}>
                      {initials}
                    </div>
                  </div>
                  ...
                  <div className="chat-item-details">
                    ...
                    <div className="chat-item-row" style={{ marginTop: '4px' }}>
                      <span className="chat-message-preview">{isContactAdmin ? 'Ban Quản trị viên' : c.email}</span>
                    </div>
                    {!isContactAdmin && (
                      <div style={{ marginTop: '6px' }}>
                        <span className="chat-booth-tag">Gian {getCustomerBooth(c.id)}</span>
                      </div>
                    )}
                  </div>
                </div>
            ...
            {contacts.length === 0 && (
              <div style={{ padding: '20px', textAlign: 'center', color: 'var(--color-slate-400)' }}>
                Chưa có khách thuê nào thuộc khu vực của bạn.
              </div>
            )}
          </div>
        </div>
              ...
              <div className="chat-main-header">
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                  <div className="chat-avatar" style={{ background: 'var(--color-primary-gradient)', color: '#FFFFFF' }}>
                    {activeContact.fullName ? activeContact.fullName.split(' ').map(n => n[0]).join('').slice(0, 2).toUpperCase() : 'KH'}
                  </div>
                  <div>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginTop: '2px' }}>
                      {activeContact.role !== 'ROLE_ADMIN' && (
                        <span className="chat-booth-pill" style={{ margin: 0 }}>Gian {getCustomerBooth(activeContact.id)}</span>
                      )}
                ...
                <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                  ...
                  <button 
                    ...
                    onClick={() => Swal.fire({
                      ...
                      html: `<b>Email:</b> ${activeContact.email}<br/><b>Số điện thoại:</b> ${activeContact.phone || 'Chưa có'}<br/><b>Vai trò:</b> ${activeContact.role}`,
                      icon: 'info'
                    })}
                  >
                    Xem Hồ sơ
